...
const { sendEmail, sendPilotApprovalNotification } = require('../utils/notifications');
...
router.get('/dashboard', auth, authorize('admin'), async (req, res, next) => {
  try {
    ...
    const stats = {
      ...
      pilots: {
        total: await Pilot.countDocuments(),
        approved: await Pilot.countDocuments({ isApproved: true }),
        pending: await Pilot.countDocuments({ isApproved: false }),
        active: await Pilot.countDocuments({ isActive: true }),
        available: await Pilot.countDocuments({ isAvailable: true, isApproved: true }),
        onDelivery: await Pilot.countDocuments({ currentOrder: { $ne: null } })
      }
    };
    ...
    const pendingPilots = await Pilot.find({ isApproved: false })
      .select('pilotId name phoneNumber vehicleDetails createdAt')
      .limit(5)
      .sort({ createdAt: -1 });
...
router.get('/dashboard/stats', auth, authorize('admin'), async (req, res, next) => {
  try {
    ...
    const openSupportTickets = await Ticket.countDocuments({ 
      status: { $in: ['open', 'in_progress'] } 
    });
    const urgentSupportTickets = await Ticket.countDocuments({ 
      priority: 'urgent',
      status: { $ne: 'closed' }
    });

    // Get support tickets this month and last month for growth calculation
    const supportTicketsThisMonth = await Ticket.countDocuments({
      createdAt: { $gte: startOfMonth }
    });
    const supportTicketsLastMonth = await Ticket.countDocuments({
      createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth }
    });
    const supportTicketGrowth = calculateGrowth(supportTicketsThisMonth, supportTicketsLastMonth);
...
// Add this new route after the existing analytics route
router.get('/analytics/products', auth, authorize('admin'), async (req, res, next) => {
  try {
    ...
    const productStats = await Product.aggregate([
      {
        $group: {
          ...
          approvedProducts: { $sum: { $cond: ['$isApproved', 1, 0] } },
...
const topSuppliers = await Product.aggregate([
  { $match: { isActive: true, isApproved: true, supplier: { $exists: true, $ne: null } } },
  {
    $group: {
      _id: '$supplier',
      productCount: { $sum: 1 },
...
router.get('/orders/:orderId', auth, authorize('admin'), [
  ...
  try {
    ...
    const { orderId } = req.params;
      ...
      .populate('supplier', 'companyName supplierId contactPersonName email phoneNumber businessAddress rating')
      .populate('items.product', 'name category brand hsnCode images pricing')
      .lean();
...
if (!order) {
    ...
    // Ensure productSnapshot images are available
...
router.get('/products/:productId', auth, authorize('admin'), [
  ...
  try {
    ...
    const product = await Product.findById(productId)
            .populate('supplier', 'companyName email phoneNumber companyAddress city state pincode rating supplierId tradeOwnerName')  
            .lean();
...
router.delete('/products/:productId', auth, authorize('admin'), [
  ...
  try {
    ...
    // Send email notification to supplier if reason provided
    if (product.supplier && reason) {
      try {
        const { sendEmail } = require('../utils/notifications');
        await sendEmail(
          product.supplier.email,
          'Product Deleted by Admin - AggreKart',
          `Dear ${product.supplier.companyName || 'Supplier'},

Your product "${product.name}" (ID: ${product._id}) has been removed from our platform.
...
AggreKart Admin Team`,
    ...
    res.json({
      success: true,
      message: 'Product deleted successfully',
      emailSent: !!(product.supplier && reason)
    });
...
router.get('/membership-config', auth, authorize('admin'), async (req, res, next) => {
  try {
    console.log('🔍 Admin getting membership configurations...');

    let configs = await MembershipConfig.find().sort({ tier: 1 });

    // If no configurations exist, create default ones
    if (!configs.length) {
      console.log('📝 Creating default membership configurations...');
      const defaultConfigs = MembershipConfig.getDefaultConfig();
      
      for (const configData of defaultConfigs) {
        const config = new MembershipConfig({
          ...configData,
      ...
      configs = await MembershipConfig.find().sort({ tier: 1 });
    }
    ...
    console.error('❌ Error getting membership configs:', error);
    next(error);
  }
});

// @route   PUT /api/admin/membership-config/:tier
// @desc    Update membership tier configuration
// @access  Private (Admin)
router.put('/membership-config/:tier', [
  auth,
  authorize('admin'),
  body('requirements.minOrders').isInt({ min: 0 }).withMessage('Minimum orders must be a non-negative integer'),
  body('requirements.minSpending').isFloat({ min: 0 }).withMessage('Minimum spending must be a non-negative number'),
  body('benefits.discountPercentage').isFloat({ min: 0, max: 50 }).withMessage('Discount percentage must be between 0 and 50'),
  body('benefits.freeDeliveryThreshold').isFloat({ min: 0 }).withMessage('Free delivery threshold must be non-negative'),
  body('benefits.aggreCoinsMultiplier').isFloat({ min: 1, max: 5 }).withMessage('Coins multiplier must be between 1 and 5'),
  body('benefits.prioritySupport').isBoolean().withMessage('Priority support must be boolean'),
  body('benefits.exclusiveDeals').isBoolean().withMessage('Exclusive deals must be boolean'),
  body('benefits.earlyAccess').isBoolean().withMessage('Early access must be boolean')
], async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    console.log(`🔧 Admin updating ${tier} tier configuration...`);
    ...
    if (!config) {
      return res.status(404).json({
        success: false,
        message: `Configuration for ${tier} tier not found`
      });
    }

    // Get all configurations to validate tier progression
    const allConfigs = await MembershipConfig.find().sort({ tier: 1 });
    
    try {
      config.validateTierProgression(allConfigs);
    ...
    console.log(`✅ ${tier} tier configuration updated successfully`);

    res.json({
      success: true,
      message: `${tier} tier configuration updated successfully`,
      data: config
    });
...
// @route   POST /api/admin/membership-config/reset-defaults
// @desc    Reset all membership configurations to defaults
// @access  Private (Admin)
router.post('/membership-config/reset-defaults', auth, authorize('admin'), async (req, res, next) => {
  try {
    console.log('🔄 Admin resetting membership configurations to defaults...');

    // Delete existing configurations
    await MembershipConfig.deleteMany({});
    ...
    console.log('✅ Membership configurations reset to defaults');

    res.json({
      success: true,
      message: 'Membership configurations reset to defaults successfully',
      data: createdConfigs
    });

  } catch (error) {
    console.error('❌ Error resetting membership configs:', error);
    next(error);
  }
});

// @route   GET /api/admin/membership-stats
// @desc    Get membership tier statistics
// @access  Private (Admin)
router.get('/membership-stats', auth, authorize('admin'), async (req, res, next) => {
  try {
    console.log('📊 Admin getting membership statistics...');
    ...
    // Get order-based tier eligibility
    const orderStats = await User.aggregate([
      { $match: { role: 'customer' } },
      {
        $project: {
          ...
          calculatedTier: {
            $switch: {
              branches: [
                { case: { $and: [{ $gte: ['$orderCount', 50] }, { $gte: ['$totalOrderValue', 200000] }] }, then: 'platinum' },
                { case: { $and: [{ $gte: ['$orderCount', 20] }, { $gte: ['$totalOrderValue', 50000] }] }, then: 'gold' }
              ],
              default: 'silver'
            }
          }
      ...
      },
      {
        $group: {
          ...
          correctTierUsers: { $sum: { $cond: [{ $eq: ['$currentTier', '$calculatedTier'] }, 1, 0] } },
          tierMismatches: { $sum: { $cond: [{ $ne: ['$currentTier', '$calculatedTier'] }, 1, 0] } }
        }
      }
    ]);

    // Format tier statistics
    ...
    const response = {
      tierDistribution: formattedStats,
      systemHealth: {
        totalCustomers: orderStats[0]?.totalCustomers || 0,
        correctTierUsers: orderStats[0]?.correctTierUsers || 0,
        tierMismatches: orderStats[0]?.tierMismatches || 0,
        accuracyPercentage: orderStats[0]?.totalCustomers > 0 
          ? ((orderStats[0]?.correctTierUsers / orderStats[0]?.totalCustomers) * 100).toFixed(1)
          : 100
      }
    };
...
// Add these routes to the existing admin.js file (around line 6100)

// Supplier Loyalty & Promotion Management Routes

// @route   GET /api/admin/supplier-promotions
// @desc    Get all supplier promotions with filters
// @access  Private (Admin)
router.get('/supplier-promotions', auth, authorize('admin'), [
  query('page').optional().isInt({ min: 1 }).withMessage('Page must be positive'),
  query('limit').optional().isInt({ min: 1, max: 50 }).withMessage('Limit must be between 1 and 50'),
  query('status').optional().isIn(['draft', 'pending_approval', 'active', 'paused', 'rejected', 'expired']).withMessage('Invalid status'),
  query('supplier').optional().isMongoId().withMessage('Invalid supplier ID'),
  query('type').optional().isIn(['discount', 'coupon', 'free_delivery', 'bulk_discount', 'seasonal', 'referral']).withMessage('Invalid promotion type')
], async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    const { page = 1, limit = 20, status, supplier, type } = req.query;
    ...
    console.log('🔍 Admin fetching supplier promotions with filters:', filter);

    // Get promotions with pagination
    const promotions = await SupplierPromotion.find(filter)
      .populate('supplier', 'companyName contactPersonName state city')
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit);
    ...
    const stats = await SupplierPromotion.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    console.error('❌ Admin supplier promotions fetch error:', error);
    next(error);
  }
});

// @route   PATCH /api/admin/supplier-promotions/:promotionId/review
// @desc    Approve or reject supplier promotion
// @access  Private (Admin)
router.patch('/supplier-promotions/:promotionId/review', auth, authorize('admin'), [
  param('promotionId').isMongoId().withMessage('Invalid promotion ID'),
  ...
  try {
    ...
    const promotion = await SupplierPromotion.findById(promotionId)
      .populate('supplier', 'companyName contactPersonName user');
    ...
    if (promotion.status !== 'pending_approval') {
      return res.status(400).json({
        success: false,
        message: 'Only pending promotions can be reviewed'
      });
    }
    ...
    if (action === 'approve') {
      ...
      if (promotion.validity.startDate > new Date()) {
    ...
    try {
      if (promotion.supplier.user) {
        const supplier = await User.findById(promotion.supplier.user);
        if (supplier && supplier.email) {
          const subject = `Promotion ${action === 'approve' ? 'Approved' : 'Rejected'}: ${promotion.title}`;
          const html = `
            <h2>Promotion ${action === 'approve' ? 'Approved' : 'Rejected'}</h2>
            <p>Dear ${promotion.supplier.companyName},</p>
            <p>Your promotion "<strong>${promotion.title}</strong>" has been ${action}d by our admin team.</p>
            ${reason ? `<p><strong>Review Notes:</strong> ${reason}</p>` : ''}
            <p>You can view and manage your promotions in your supplier dashboard.</p>
            <p>Thank you,<br>Aggrekart Team</p>
          `;
          
          await sendEmail(supplier.email, subject, html);
          console.log(`📧 ${action} notification sent to supplier`);
        }
      }
    } catch (notificationError) {
    ...
    console.error('❌ Admin promotion review error:', error);
    next(error);
  }
});

// @route   GET /api/admin/promotion-stats
// @desc    Get promotion statistics for admin dashboard
// @access  Private (Admin)
router.get('/promotion-stats', auth, authorize('admin'), [
  ...
  try {
    ...
    console.log(`🔍 Admin fetching promotion stats for last ${days} days...`);

    // Overall promotion statistics
    const overallStats = await SupplierPromotion.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    // Recent promotions (last X days)
    const recentStats = await SupplierPromotion.aggregate([
      {
        $match: {
    ...
    // Top performing suppliers
    const topSuppliers = await SupplierPromotion.aggregate([
      {
        $match: {
          status: 'active',
    ...
    const typeDistribution = await SupplierPromotion.aggregate([
      {
        $group: {
          _id: '$type',
          count: { $sum: 1 },
    ...
    console.error('❌ Admin promotion stats error:', error);
    next(error);
  }
});

// @route   GET /api/admin/customer-type-analytics
// @desc    Get customer type analytics for promotion targeting
// @access  Private (Admin)
router.get('/customer-type-analytics', auth, authorize('admin'), [
  query('days').optional().isInt({ min: 1, max: 365 }).withMessage('Days must be between 1 and 365')
], async (req, res, next) => {
  try {
    const errors = validationResult(req);
    ...
    console.log(`🔍 Admin fetching customer type analytics for last ${days} days...`);

    // Customer type distribution and behavior
    const customerTypeAnalytics = await Order.aggregate([
      {
        $match: {
          createdAt: { $gte: startDate },
          status: { $ne: 'cancelled' }
    ...
    const stateAnalytics = await Order.aggregate([
      {
        $match: {
          createdAt: { $gte: startDate },
          status: { $ne: 'cancelled' }
      ...
      },
      {
        $project: {
          ...
          totalRevenue: 1,
          uniqueCustomers: { $size: '$uniqueCustomers' },
          avgRevenuePerCustomer: { $divide: ['$totalRevenue', { $size: '$uniqueCustomers' }] }
        }
      },
      { $sort: { totalRevenue: -1 } }
    ]);
    ...
    res.json({
      ...
      data: {
        ...
        stateAnalytics,
        summary: {
          totalAnalyzedOrders: customerTypeAnalytics.reduce((sum, item) => sum + item.totalOrders, 0),
          totalRevenue: customerTypeAnalytics.reduce((sum, item) => sum + item.totalRevenue, 0),
          totalCustomers: customerTypeAnalytics.reduce((sum, item) => sum + item.uniqueCustomers, 0)
        },
        dateRange: {
          from: startDate,
          to: new Date(),
    ...
    console.error('❌ Admin customer type analytics error:', error);
    next(error);
  }
});

// @route   POST /api/admin/system-promotions
// @desc    Create system-wide promotions that apply to all suppliers
// @access  Private (Admin)
router.post('/system-promotions', auth, authorize('admin'), [
  body('title').notEmpty().withMessage('Title is required'),
  body('description').notEmpty().withMessage('Description is required'),
  body('type').isIn(['discount', 'free_delivery', 'seasonal', 'festival']).withMessage('Invalid promotion type'),
  body('benefits.discountType').isIn(['percentage', 'fixed']).withMessage('Invalid discount type'),
  ...
  body('targeting.customerTypes').optional().isArray().withMessage('Customer types must be an array'),
  body('targeting.states').optional().isArray().withMessage('States must be an array'),
  body('targeting.membershipTiers').optional().isArray().withMessage('Membership tiers must be an array')
], async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    console.log('🔍 Admin creating system promotion:', promotionData.title);

    // Validate dates
    const startDate = new Date(promotionData.validity.startDate);
    const endDate = new Date(promotionData.validity.endDate);
    ...
    // Create system promotion (applies to all suppliers)
    const systemPromotion = new SupplierPromotion({
      ...promotionData,
      supplier: null, // null means system-wide
      isSystemPromotion: true,
      status: 'active', // Admin promotions are automatically active
      isActive: startDate <= new Date(),
      createdBy: req.user._id,
      reviewedBy: req.user._id,
      reviewedAt: new Date()
    });

    await systemPromotion.save();

    console.log('✅ System promotion created:', systemPromotion._id);

    res.status(201).json({
      success: true,
      message: 'System promotion created successfully',
      data: systemPromotion
    });

  } catch (error) {
    console.error('❌ Admin system promotion creation error:', error);
    next(error);
  }
});
// ...existing code... (add this route after the existing supplier-promotions routes, around line 6300)

// @route   POST /api/admin/supplier-promotions/create
// @desc    Create supplier promotion as admin (bypasses approval)
// @access  Private (Admin)
// ...existing code... (add this route after the existing supplier-promotions routes, around line 6350)

// @route   POST /api/admin/supplier-promotions/create
// @desc    Create supplier promotion as admin (bypasses approval)
// @access  Private (Admin)
// Replace the validation array (around line 6670):

router.post('/supplier-promotions/create', auth, authorize('admin'), [
  body('supplierId').isMongoId().withMessage('Valid supplier ID is required'),
  body('title').trim().isLength({ min: 3, max: 100 }).withMessage('Title must be 3-100 characters'),
  body('description').optional().isString().withMessage('Description must be a string'),
  body('type').optional().isIn(['discount', 'coupon', 'free_delivery', 'bulk_discount', 'seasonal', 'referral']).withMessage('Invalid promotion type'),
  
  // Benefits validation
  body('discountType').isIn(['percentage', 'fixed_amount', 'free_delivery', 'coins_multiplier']).withMessage('Invalid discount type'),
  ...
  body('freeDeliveryRadius').optional().isFloat({ min: 0 }).withMessage('Free delivery radius must be positive'),
  body('coinsMultiplier').optional().isFloat({ min: 1, max: 5 }).withMessage('Coins multiplier must be 1-5'),
  
  // Targeting validation
  body('customerTypes').optional().isArray().withMessage('Customer types must be an array'),
  body('membershipTiers').optional().isArray().withMessage('Membership tiers must be an array'),
  body('newCustomersOnly').optional().isBoolean().withMessage('New customers only must be boolean'),
  body('returningCustomersOnly').optional().isBoolean().withMessage('Returning customers only must be boolean'),
  
  // Conditions validation
  body('minOrderAmount').optional().isFloat({ min: 0 }).withMessage('Minimum order amount must be positive'),
  body('maxOrderValue').optional().isFloat({ min: 0 }).withMessage('Maximum order value must be positive'),
  body('minQuantity').optional().isInt({ min: 1 }).withMessage('Minimum quantity must be at least 1'),
  body('categories').optional().isArray().withMessage('Categories must be an array'),
  
  // Usage limits validation
  body('totalLimit').optional().isInt({ min: 1 }).withMessage('Total limit must be at least 1'),
  body('perUserLimit').optional().isInt({ min: 1 }).withMessage('Per user limit must be at least 1'),
  body('dailyLimit').optional().isInt({ min: 1 }).withMessage('Daily limit must be at least 1'),
  
  // Validity validation
  body('validFrom').optional().isISO8601().withMessage('Valid from date must be valid ISO date'),
  body('validUntil').optional().isISO8601().withMessage('Valid until date must be valid ISO date'),
  ...
  body('discountValue').custom((value, { req }) => {
    if (req.body.discountType === 'percentage' && (value < 0 || value > 100)) {
      throw new Error('Percentage discount must be between 0-100');
    }
    return true;
  }),
  ...
  // Replace the route body after validation (around line 6690):
...
try {
  ...
  console.log('🔍 Admin creating comprehensive supplier promotion:', { 
    supplierId, 
    title, 
    discountType, 
    discountValue 
  ...
  const supplier = await Supplier.findById(supplierId).populate('user', 'name email');
  if (!supplier) {
    return res.status(404).json({
      success: false,
  ...
  if (!supplier.isApproved || !supplier.isActive) {
    return res.status(400).json({
      success: false,
      message: 'Supplier must be approved and active'
    });
  }
  ...
  const endDate = validUntil ? new Date(validUntil) : new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000); // 30 days default
  ...
  const promotionId = `ADMIN_PROMO_${Date.now()}_${supplierId.slice(-6)}`;

  // Create comprehensive promotion data
  const promotionData = {
    promotionId,
    supplier: supplierId,
    title,
    description: description || `${discountValue}${discountType === 'percentage' ? '%' : ''} discount promotion`,
    type: type || 'discount',
    ...
    targeting: {
      ...
      returningCustomersOnly: returningCustomersOnly || false
    },
    ...
    // Benefits (REQUIRED FIELDS)
    benefits: {
      discountType: discountType,
      discountValue: parseFloat(discountValue) || 0,
      maxDiscount: maxDiscount ? parseFloat(maxDiscount) : null,
      freeDeliveryRadius: freeDeliveryRadius ? parseFloat(freeDeliveryRadius) : 0,
      coinsMultiplier: coinsMultiplier ? parseFloat(coinsMultiplier) : 1,
      additionalBenefits: {
    ...
    budget: {
      ...
      usedBudget: 0,
      remainingBudget: totalBudget ? parseFloat(totalBudget) : 0
    },
    ...
    // Auto-approval since created by admin
    reviewStatus: 'approved',
    reviewedBy: req.user._id,
    reviewedAt: now,
    reviewNotes: 'Created directly by admin - auto-approved'
  };

  console.log('🔍 Creating promotion with data:', {
    promotionId,
    discountType: promotionData.benefits.discountType,
    discountValue: promotionData.benefits.discountValue,
    benefits: promotionData.benefits
  });
  ...
  await promotion.populate([
    { path: 'supplier', select: 'companyName contactPersonName state city' },
    { path: 'createdBy', select: 'name email role' }
  ]);

  console.log('✅ Admin-created supplier promotion saved:', promotion._id);

  // Send notification to supplier (optional)
  try {
    if (supplier.user && supplier.user.email) {
      await sendEmail({
        to: supplier.user.email,
        subject: 'New Promotion Created for Your Business',
        template: 'supplier-promotion-created',
        data: {
          supplierName: supplier.contactPersonName || supplier.companyName,
          promotionTitle: title,
          discountValue: discountValue,
          discountType: discountType,
          validFrom: startDate.toLocaleDateString(),
          validUntil: endDate.toLocaleDateString(),
          loginUrl: `${process.env.FRONTEND_URL}/supplier/dashboard`
        }
      });
    }
  } catch (emailError) {
  ...
  res.status(201).json({
    success: true,
    message: 'Supplier promotion created successfully',
    data: { 
      promotion: {
        _id: promotion._id,
  ...
  console.error('❌ Admin create supplier promotion failed:', error);
  next(error);
}
});
...
// @route   GET /api/admin/pilots
// @desc    Get all pilots with filters and pagination
// @access  Private (Admin)
router.get('/pilots', auth, authorize('admin'), [
  query('page').optional().isInt({ min: 1 }).withMessage('Page must be a positive integer'),
  query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('Limit must be between 1-100'),
  query('status').optional().isIn(['pending', 'approved', 'rejected', 'suspended']).withMessage('Invalid status'),
  query('availability').optional().isBoolean().withMessage('Availability must be boolean'),
  ...
  try {
    ...
    if (req.query.status) {
      ...
      else if (req.query.status === 'suspended') query.isActive = false;
    }

    if (req.query.availability !== undefined) {
      query.isAvailable = req.query.availability === 'true';
    }
    ...
    // Get pilots with pagination
    const pilots = await Pilot.find(query)
      .select('-documents -tempOTP -otpExpiry')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const total = await Pilot.countDocuments(query);
    ...
    res.json({
      ...
      data: {
        pilots: pilots.map(pilot => ({
          ...
          onlineStatus: new Date() - pilot.currentLocation?.lastUpdated < 10 * 60 * 1000 ? 'online' : 'offline'
        })),
        pagination: {
          currentPage: page,
...
// @route   POST /api/admin/pilots/:pilotId/approve
// @desc    Approve a pilot
// @access  Private (Admin)
router.post('/pilots/:pilotId/approve', auth, authorize('admin'), [
  ...
  try {
    ...
    if (pilot.isApproved) {
      return next(new ErrorHandler('Pilot is already approved', 400));
    }

    // Approve pilot
    pilot.isApproved = true;
    pilot.isActive = true;
    pilot.approvedBy = req.user._id;
    pilot.approvedAt = new Date();
    
    if (notes) {
      pilot.approvalNotes = notes;
    }
    ...
    try {
      await sendPilotApprovalNotification(pilot);
    } catch (error) {
...
// @route   GET /api/admin/pilots/analytics
// @desc    Get pilot fleet analytics
// @access  Private (Admin)
router.get('/pilots/analytics', auth, authorize('admin'), async (req, res, next) => {
  try {
    // Vehicle type distribution
    const vehicleDistribution = await Pilot.aggregate([
      { $match: { isApproved: true } },
      { $group: { _id: '$vehicleDetails.vehicleType', count: { $sum: 1 } } }
    ]);

    // Pilot performance analytics
    const performanceAnalytics = await Order.aggregate([
      { $match: { 'delivery.pilotAssigned': { $exists: true }, status: 'delivered' } },
      {
        $lookup: {
          from: 'pilots',
          localField: 'delivery.pilotAssigned',
          foreignField: '_id',
          as: 'pilot'
        }
      },
      { $unwind: '$pilot' },
      {
        $group: {
          ...
          avgDeliveryValue: { $avg: '$pricing.totalAmount' },
          topPerformers: {
            $push: {
              pilotId: '$pilot.pilotId',
              pilotName: '$pilot.name',
              deliveryValue: '$pricing.totalAmount'
            }
          }
        }
      }
    ...
    const signupTrends = await Pilot.aggregate([
      {
        $group: {
          _id: {
            year: { $year: '$createdAt' },
    ...
    res.json({
      ...
      data: {
        fleetOverview: {
          totalPilots: await Pilot.countDocuments(),
          activePilots: await Pilot.countDocuments({ isActive: true }),
          availablePilots: await Pilot.countDocuments({ isAvailable: true, isApproved: true }),
          onDeliveryPilots: await Pilot.countDocuments({ currentOrder: { $ne: null } })
        },
        vehicleDistribution,
